...
public class SkillButton extends Button {
    ...
    public SkillButton(int x, int y, String skillId) {
        super(new Builder(Component.literal(""), onPress -> RequestLevelUp.send(skillId))
                .pos(x, y)
                .size(79, 32));
    ...
    protected void extractContents(@NotNull GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        Identifier configuredIcon = builtIn == null ? null : Configuration.getBuiltInSkillIcon(builtIn);
        if (configuredIcon != null) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, configuredIcon, getX() + 6, getY() + 8, 0, 0, 16, 16, 16, 16);
        } else {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, SkillScreen.RESOURCES, getX() + 6, getY() + 8, u, v, 16, 16, 256, 256);
        }
        ...
        drawScaledToFitText(guiGraphics, font, getDisplayNameComponent(), nameX, nameY, maxNameWidth, 0xFFFFFFFF);
    ...
    private int getSkillIconU(int level, int maxLevel) {
        ...
        int filledStep = Math.max(0, Math.min(3, (int) Math.ceil((double) level * 4 / safeMaxLevel) - 1));
    ...
    private void drawScaledToFitText(GuiGraphicsExtractor guiGraphics, Font font, Component text, int x, int y, int maxWidth, int color) {
        ...
        net.minecraft.network.chat.FormattedText clipped = font.substrByWidth(text, Math.max(0, maxWidth - ellipsisWidth));
        net.minecraft.network.chat.FormattedText fitted = net.minecraft.network.chat.FormattedText.composite(
                clipped,
                net.minecraft.network.chat.FormattedText.of("…")
        );
        guiGraphics.text(font, net.minecraft.locale.Language.getInstance().getVisualOrder(fitted), x, y, color, false);
    }
}